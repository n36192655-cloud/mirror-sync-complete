...
export const EXTRA_ASSISTANT_TOOLS = [
 ...
 { type: "function", function: { name: "get_project_efficiency", description: "تحليل كفاءة المشروع خلال فترة: الإنتاج المسجل، الاستهلاك المعتمد، وفجوة الإنتاج والاستهلاك، والتحصيل. لا تسمِّ الفجوة تلقائياً فاقداً ظاهرياً أو تسرباً أو NRW؛ السبب يحتاج Water Balance مكتمل وبيانات إضافية. إذا كان الاستهلاك أكبر من الإنتاج فهذه مشكلة جودة بيانات.", parameters: { type: "object", properties: { from: { type: "string" }, to: { type: "string" } }, required: ["from", "to"], additionalProperties: false } } },
