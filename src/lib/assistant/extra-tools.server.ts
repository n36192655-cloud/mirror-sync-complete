...
export const EXTRA_ASSISTANT_TOOLS = [
 { type: "function", function: { name: "get_customer_period_summary", description: "كشف حساب تحليلي دقيق لمشترك خلال فترة محددة: الاستهلاك المعتمد فقط، إجمالي الفواتير، أعلى وأقل فاتورة، المدفوعات المعتمدة، الفواتير غير المسددة، والرصيد الحالي.", parameters: { type: "object", properties: periodProps, required: ["customer_id", "from", "to"], additionalProperties: false } } },
 { type: "function", function: { name: "get_project_efficiency", description: "تحليل كفاءة المشروع خلال فترة: الإنتاج المسجل، الاستهلاك المعتمد، الفاقد المائي الظاهري، نسبة الفاقد، والتحصيل. إذا كان الاستهلاك أكبر من الإنتاج فهذه مشكلة جودة بيانات وليست فاقداً.", parameters: { type: "object", properties: { from: { type: "string" }, to: { type: "string" } }, required: ["from", "to"], additionalProperties: false } } },
 { type: "function", function: { name: "list_unpaid_bills", description: "قائمة دقيقة للفواتير التي لم تُسدّد بالكامل. تفحص حتى 200 نتيجة قبل تطبيق حد العرض، لذلك لا تعاني من مشكلة تطبيق limit قبل تصفية غير المسدد.", parameters: { type: "object", properties: { customer_id: { type: "string" }, from: { type: "string" }, to: { type: "string" }, limit: { type: "number" } }, additionalProperties: false } } },
] as const;
...
async function getProjectEfficiency(supabase: DB, args: Record<string, unknown>): Promise<ExtraToolResult> {
 const from=typeof args.from==="string"?args.from:""; const to=typeof args.to==="string"?args.to:"";
 if(!/^\d{4}-\d{2}-\d{2}$/.test(from)||!/^\d{4}-\d{2}-\d{2}$/.test(to)||from>to) return fail("نطاق التاريخ غير صالح.");
 const [{data:production,error:productionError},{data:readings,error:readingsError},{data:bills,error:billsError},{data:payments,error:paymentsError}]=await Promise.all([
  ...
  supabase.from("water_readings").select("consumption,status,reading_date").gte("reading_date",from).lte("reading_date",`${to}T23:59:59`),
 ...
 const produced=(production??[]).reduce((s,p)=>s+Math.max(n(p.produced_m3),0),0); const consumed=(readings??[]).filter(r=>r.status==="approved").reduce((s,r)=>s+Math.max(n(r.consumption),0),0); const billed=(bills??[]).reduce((s,b)=>s+n(b.total),0); const collected=(payments??[]).filter(p=>p.status==="approved").reduce((s,p)=>s+n(p.amount),0); const balanceGap=produced-consumed; const lossPct=produced>0?Math.round((Math.max(balanceGap,0)/produced)*1000)/10:null; const collectionPct=billed>0?Math.round((collected/billed)*1000)/10:0; const dataQuality=balanceGap<0?"تنبيه: الاستهلاك المعتمد أكبر من الإنتاج المسجل؛ راجع اكتمال بيانات الإنتاج والقراءات قبل تفسير الفاقد.":produced===0?"لا يوجد إنتاج مسجل في الفترة المحددة، لذلك لا يمكن حساب نسبة الفاقد.":"الفرق الموجب مؤشر فاقد مائي ظاهري، ولا يحدد وحده سبب الفاقد.";
