...
function validate(input: unknown): VisionInput {
  ...
  const knownMeterNumber =
    typeof obj.knownMeterNumber === "string" ? obj.knownMeterNumber.trim().slice(0, 40) : undefined;
...
- أعد JSON فقط.`;
    ...
    async function runPass(): Promise<Pass> {
      ...
      const meterNumber = typeof parsed.meterNumber === "string" && parsed.meterNumber.trim()
