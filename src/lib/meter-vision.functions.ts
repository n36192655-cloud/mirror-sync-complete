...
- أعد JSON فقط.`;
    ...
    async function runPass(): Promise<Pass> {
      ...
      try {
        response = (await geminiChat(apiKey, { messages: [
          { role: "system", content: system },
          { role: "user", content: [
            { type: "text", text: `حلّل صورة عداد المياه. تحقق من هوية العداد أولاً، ثم اقرأ خانات الاستهلاك فقط.\n${hints}` },
            { type: "image_url", image_url: { url: data.imageDataUrl } },
          ] },
        ], response_format: { type: "json_schema", json_schema: { name: "meter_reading", schema } } })) as typeof response;
      } catch (error) {
      ...
      catch { const match = content.match(/\{[\s\S]*\}/); if (match) { try { parsed = JSON.parse(match[0]) as Record<string, unknown>; } catch { parsed = {}; } } }
      ...
      const readingValue = digitOnly.length >= 1 && digitOnly.length <= 12 && digitOnly.length === compactDigits.length ? Number(digitOnly) : null;
      ...
      return { readingValue, readingDigits, confidence, meterNumber, otherNumbers, ambiguous: parsed.ambiguous === true || readingValue == null, serialMatch };
    }
