...
export interface MeterOcrResult {
  rawText: string; tokens: OcrToken[]; meterNumberMatch: string | null; readingCandidate: string | null;
  readingValue: number | null; readingConfidence: number; readingAmbiguous: boolean; otherTokens: OcrToken[];
}

const ARABIC_DIGITS = /[٠-٩۰-۹]/g;
export function normalizeDigits(input: string): string {
  return input.replace(ARABIC_DIGITS, (d) => { const code = d.charCodeAt(0); return code >= 0x0660 && code <= 0x0669 ? String(code - 0x0660) : String(code - 0x06f0); });
}
export function normalizeSerial(v: string): string {
  return normalizeDigits(v).normalize("NFKC").trim().toUpperCase().replace(/[\u2010-\u2015\u2212]/g, "-").replace(/[\s_-]+/g, "-").replace(/^-+|-+$/g, "");
}
const DATE_RE = /^(19|20)\d{2}$|^\d{1,2}[./-]\d{1,2}([./-]\d{2,4})?$/;
const UNIT_RE = /^(m3|m³|cbm|kwh|lt|l|kg|bar|°c|mm|cm)$/i;
function classify(text: string, knownSerialNorm: string, isReadingCandidate: boolean): OcrToken["kind"] {
...
/** يحول الملف إلى data URL دون إعادة ترميز أو ضغط؛ البايتات الأصلية لا تتغير. */
export function fileToDataUrl(file: Blob): Promise<string> {
  return new Promise((resolve, reject) => { const reader = new FileReader(); reader.onload = () => resolve(String(reader.result)); reader.onerror = () => reject(reader.error ?? new Error("تعذر قراءة الصورة")); reader.readAsDataURL(file); });
}
...
export async function recognizeMeterImage(image: Blob | File | string, options: RecognizeOptions = {}): Promise<MeterOcrResult> {
  ...
  try {
    const input = await preprocess(image); const general = await worker.recognize(input as never, {}, { text: true, blocks: true } as never); const rawText = general.data.text ?? ""; const generalWords = flattenWords(general.data);
    ...
    const knownSerialNorm = options.knownMeterNumber ? normalizeSerial(options.knownMeterNumber) : "";
    const fallback: FlatWord[] = !generalWords.length && !digitWords.length ? rawText.split(/\s+/).filter(Boolean).map((t) => ({ text: t.trim(), confidence: 0, height: 0 })) : [];
    ...
    const serialProven = !knownSerialNorm || cleaned.some((w) => normalizeSerial(w.text) === knownSerialNorm);
    const candidates = cleaned.map((w) => ({ ...w, shape: readingShape(w.text) })).filter((w) => w.shape.ok && w.shape.value != null && w.shape.value >= 0 && normalizeSerial(w.text) !== knownSerialNorm && !excluded.has(normalizeSerial(w.text)) && !UNIT_RE.test(w.text.trim()) && !DATE_RE.test(normalizeDigits(w.text.trim())));
    ...
    const filteredPrev = scored.filter((c) => prev == null || c.shape.value !== prev); const seen = new Set<number>(); const usable = filteredPrev.filter((c) => { const v = c.shape.value as number; if (seen.has(v)) return false; seen.add(v); return true; });
    const best = usable[0] ?? null; const second = usable[1] ?? null; const readingAmbiguous = !!best && !!second && second.score >= best.score * 0.97;
    const tokens: OcrToken[] = cleaned.map((w) => ({ text: w.text, confidence: Math.round(w.confidence), height: Math.round(w.height), kind: classify(w.text, knownSerialNorm, best ? w.text === best.text : false) }));
    const meterNumberMatch = serialProven ? (options.knownMeterNumber ?? null) : null;
    if (knownSerialNorm && !serialProven) throw new Error(`عذراً، تعذر إثبات رقم العداد المرتبط (${options.knownMeterNumber}). أعد تصوير الرقم كاملاً وبوضوح.`);
