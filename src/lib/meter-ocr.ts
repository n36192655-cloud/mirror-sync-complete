 ...
 * لا يتم قبول رقم غير مقروء أو تخمينه. القراءة الحالية تُستخرج فقط من
 * المرشحين الرقميين الواضحين، مع رفض الحالات الملتبسة.
 */
...
/**
 * تطبيع محافظ لرقم العداد. نزيل فقط اختلافات العرض الشائعة، ولا نحذف
 * أصفاراً أو أحرفاً ولا نستخدم prefix/suffix/substring matching.
 */
...
export async function recognizeMeterImage(image: Blob | File | string, options: RecognizeOptions = {}): Promise<MeterOcrResult> {
  ...
  try {
    ...
    try {
      ...
      const digits = await worker.recognize(input as never, {}, { text: true, blocks: true } as never);
    ...
    const fallback: FlatWord[] = generalWords.length === 0 && digitWords.length === 0
      ? rawText.split(/\s+/).filter(Boolean).map((t) => ({ text: t.trim(), confidence: 0, height: 0 }))
      : [];
    ...
    const meterNumberMatch = knownSerialNorm && cleaned.some(
      (w) => normalizeSerial(w.text) === knownSerialNorm
    ) ? (options.knownMeterNumber ?? null) : null;
