...
function classify(text: string, known: string, isReading: boolean): OcrToken["kind"] {
  const n = normalizeSerial(text);
  if (known && n === known) return "meter-number";
...
export async function recognizeMeterImage(
  ...
  try {
    ...
    const fallback =
      !generalWords.length && !digitWords.length
        ? rawText
            .split(/\s+/)
            .filter(Boolean)
    ...
    const scored = candidates
      .map((c) => {
        ...
        if (prev != null && c.shape.value != null) {
          ...
          if (Math.abs(c.shape.value - prev) <= Math.max(50, prev * 0.5)) score += 25;
        }
        return { ...c, score };
      })
    ...
    const tokens: OcrToken[] = cleaned.map((w) => ({
      ...
      kind: classify(w.text, known, best ? w.text === best.text : false),
    }));
    const meterNumberMatch = serialProven ? (options.knownMeterNumber ?? null) : null;
    if (known && !serialProven) {
      throw new Error(
        `عذراً، تعذر إثبات رقم العداد المرتبط (${options.knownMeterNumber}). أعد تصوير الرقم كاملاً وبوضوح.`,
      );
    }
    return {
      rawText,
      tokens,
      meterNumberMatch,
      readingCandidate: best?.text ?? null,
      readingValue: best?.shape.value ?? null,
