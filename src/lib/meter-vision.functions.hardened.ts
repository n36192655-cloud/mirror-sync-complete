...
async function proofKey(apiKey: string): Promise<CryptoKey> {
  return crypto.subtle.importKey("raw", encoder.encode(`MIZAN-METER-PROOF-v2:${apiKey}`).buffer, { name: "HMAC", hash: "SHA-256" }, false, ["sign"]);
}

async function signProof(payload: string, apiKey: string): Promise<string> {
  const signature = await crypto.subtle.sign("HMAC", await proofKey(apiKey), encoder.encode(payload).buffer);
  return `${b64url(encoder.encode(payload))}.${b64url(signature)}`;
}

async function sha256(bytes: Uint8Array): Promise<string> {
  const copy = new ArrayBuffer(bytes.byteLength);
  new Uint8Array(copy).set(bytes);
  return b64url(await crypto.subtle.digest("SHA-256", copy));
}
...
function profileFromRow(row: Record<string, unknown>): MeterReadingProfile {
  const displayType = typeof row.display_type === "string" ? row.display_type : null;
  const integerDigits = typeof row.integer_digits === "number" ? row.integer_digits : null;
  const decimalDigits = typeof row.decimal_digits === "number" ? row.decimal_digits : null;
  const decimalSeparator = row.decimal_separator === "." || row.decimal_separator === "," ? row.decimal_separator : null;
  const registerSemantics = row.register_semantics && typeof row.register_semantics === "object" && !Array.isArray(row.register_semantics)
    ? row.register_semantics as Record<string, unknown>
    : null;
  return {
    displayType: displayType as MeterReadingProfile["displayType"],
    integerDigits,
    decimalDigits,
    decimalSeparator,
    registerSemantics,
...
async function loadAuthoritativeContext(context: { supabase: SupabaseClient; userId: string }, data: VerificationInput) {
  ...
  const { data: meter } = await meterQuery.from("meters")
    .select("id, serial, tenant_id, display_type, integer_digits, decimal_digits, decimal_separator, register_semantics")
    .eq("id", data.meterId).maybeSingle();
  ...
  const { data: assignment } = await context.supabase.from("meter_assignments")
    .select("customer_id, meter_id, started_at, ended_at")
    .eq("tenant_id", tenantId).eq("customer_id", data.customerId).eq("meter_id", data.meterId)
    .lte("started_at", `${data.readingDate}T23:59:59.999Z`)
    .or(`ended_at.is.null,ended_at.gte.${data.readingDate}T00:00:00.000Z`)
...
function parseVisionResponse(response: unknown, expectedMeterNumber: string, previousReading: number, profile: MeterReadingProfile) {
  ...
  const rawReading = typeof parsed.readingDigits === "string" ? parsed.readingDigits : "";
  const candidate = normalizeMeterReadingCandidate(rawReading, profile);
  const readingValue = candidate?.value ?? null;
...
async function runInference(apiKey: string, data: VerificationInput, expectedMeterNumber: string, previousReading: number, profile: MeterReadingProfile) {
  ...
  const schema = {
    ...
    properties: {
      readingDigits: { type: "string" }, confidence: { type: "number" }, meterNumber: { type: "string" },
      otherNumbers: { type: "array", items: { type: "string" } }, ambiguous: { type: "boolean" },
    },
  ...
  try {
    ...
    const result = parseVisionResponse(response, expectedMeterNumber, previousReading, profile);
    const metrics: MeterPipelineMetrics = { imagePreparationMs: 0, aiInferenceMs: performance.now() - started, parseMs: 0, identityValidationMs: 0, totalServerMs: performance.now() - started };
...
export const verifyMeterImage = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }): Promise<MeterVerificationResult> => {
    const apiKey = process.env["GEMINI_API_KEY"] ?? "";
    if (!apiKey) throw new Error("GEMINI_API_KEY مفقود");
    const originalBytes = decodeImageDataUrl(data.originalImageDataUrl);
    const imageHash = await sha256(originalBytes);
    const auth = await loadAuthoritativeContext(context, data);
    const { result, metrics } = await runInference(apiKey, data, auth.meter.serial, auth.previousReading, auth.profile);
    if (result.serialMatch === "mismatch") throw new Error("رفض: هوية العداد الظاهرة في الصورة لا تطابق العداد المرتبط");
    if (result.ambiguous || result.readingValue == null) throw new Error("رفض القراءة: تعذر استخراج قراءة واضحة أو لم تسمح إعدادات العداد بتحديد الدقة");
    const payload = JSON.stringify({
      v: "2", userId: context.userId, tenantId: auth.tenantId, customerId: data.customerId, meterId: data.meterId,
      readingDate: data.readingDate, clientUuid: data.clientUuid, meterNumber: auth.meter.serial,
      readingValue: String(result.readingValue), serialMatch: result.serialMatch, imageHash,
      attemptCount: data.attemptCount, exp: Date.now() + 10 * 60 * 1000,
    });
