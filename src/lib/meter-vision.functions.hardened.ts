...
function profileFromRow(row: Record<string, unknown>): MeterReadingProfile {
  ...
  const registerSemantics = row.register_semantics && typeof row.register_semantics === "object" && !Array.isArray(row.register_semantics)
    ...
    : null;
  return { displayType: displayType as MeterReadingProfile["displayType"], integerDigits, decimalDigits, decimalSeparator, registerSemantics };
}
...
export const verifyMeterImage = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }): Promise<MeterVerificationResult> => {
    ...
    const payload = JSON.stringify({ v: "2", userId: context.userId, tenantId: auth.tenantId, customerId: data.customerId, meterId: data.meterId, readingDate: data.readingDate, clientUuid: data.clientUuid, meterNumber: auth.meter.serial, readingValue: String(result.readingValue), serialMatch: result.serialMatch, imageHash, attemptCount: data.attemptCount, exp: Date.now() + 10 * 60 * 1000 });
