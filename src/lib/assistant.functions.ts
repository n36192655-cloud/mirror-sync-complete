...
في النهاية أضف دائماً: [اقتراحات] سؤال1 | سؤال2 | سؤال3`;
    ...
    for (let step = 0; step < 6; step++) {
      ...
      try {
        payload = (await geminiChat(apiKey, { messages, tools: allTools, temperature: 0.1 })) as typeof payload;
      } catch (err) {
      ...
      if (calls.length === 0) {
        ...
        if (match) {
          ...
          suggestions.push(...(match[1] ?? "").split("|").map((s) => s.trim()).filter(Boolean).slice(0, 4));
        }
    ...
    return {
      answer: "لم أتمكن من إكمال التحليل ضمن الحد الآمن للخطوات. جرّب تقسيم السؤال إلى جزءين.",
      tables,
      tools: usedTools,
      suggestions: ["كشف حساب هذا المشترك", "أعلى فاتورة في الفترة", "المدفوعات في الفترة"],
    };
  });
