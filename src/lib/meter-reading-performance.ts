...
export function markProcessingStarted(trace: MeterReadingPerformance, at = performance.now()): void {
  ...
  if (trace.captureCompletedAt !== null && at < trace.captureCompletedAt) {
    throw new RangeError("processing cannot precede capture completion");
  }
...
export function markFieldAssigned(trace: MeterReadingPerformance, at = performance.now()): void {
  ...
  if (trace.captureCompletedAt === null || trace.processingStartedAt === null) {
    throw new Error("field assignment requires capture and processing timestamps");
  }
  if (at < trace.captureCompletedAt || at < trace.processingStartedAt) {
    throw new RangeError("field assignment cannot precede prior stages");
  }
