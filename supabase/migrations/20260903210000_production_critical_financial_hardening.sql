...
-- Additive, idempotent safeguards for production financial integrity.
BEGIN;
...
-- Freeze paid and partially-paid invoice monetary fields. Corrections must use adjustments.
...
-- Historical tariff engine: apply every slab in the saved version rather than rate_structure[0].
...
BEGIN
  ...
  -- Safe fallback for tenants without a populated historical snapshot.
...
-- Atomic, retry-safe payment creation. Pending payments reserve bill capacity.
CREATE OR REPLACE FUNCTION public.record_payment(
  _bill_id UUID, _amount NUMERIC, _method TEXT, _client_uuid TEXT DEFAULT NULL
) RETURNS UUID LANGUAGE plpgsql SECURITY DEFINER
SET search_path = pg_catalog, public, pg_temp AS $$
DECLARE
  _uid UUID := auth.uid(); _bill public.water_bills%ROWTYPE;
  _approved NUMERIC(18,3); _pending NUMERIC(18,3); _remaining NUMERIC(18,3);
...
BEGIN
  ...
  IF NOT (public.has_tenant_role(_bill.tenant_id,'collector') OR public.has_tenant_role(_bill.tenant_id,'manager')
      OR public.has_tenant_role(_bill.tenant_id,'admin') OR public.has_tenant_role(_bill.tenant_id,'accountant')
      OR public.has_tenant_role(_bill.tenant_id,'super_admin')) THEN RAISE EXCEPTION 'forbidden'; END IF;
  IF _client_uuid IS NOT NULL THEN
    SELECT id INTO _existing FROM public.payments
    WHERE tenant_id=_bill.tenant_id AND client_uuid=_client_uuid LIMIT 1;
    IF _existing IS NOT NULL THEN RETURN _existing; END IF;
  END IF;
  SELECT COALESCE(SUM(amount),0) INTO _approved FROM public.payments WHERE bill_id=_bill.id AND status='approved';
  SELECT COALESCE(SUM(amount),0) INTO _pending FROM public.payments WHERE bill_id=_bill.id AND status='pending';
  ...
  IF _amount > _remaining + 0.0005 THEN RAISE EXCEPTION 'amount exceeds remaining balance'; END IF;
...
-- Atomic approval: one immutable ledger row, exact bill paid amount, then balance rebuild.
CREATE OR REPLACE FUNCTION public.approve_payment(_payment_id UUID)
RETURNS VOID LANGUAGE plpgsql SECURITY DEFINER
SET search_path = pg_catalog, public, pg_temp AS $$
DECLARE
  _uid UUID := auth.uid(); _pay public.payments%ROWTYPE; _bill public.water_bills%ROWTYPE;
  _approved NUMERIC(18,3); _new_paid NUMERIC(18,3); _new_status TEXT;
BEGIN
  PERFORM public.assert_authenticated_context();
  IF _uid IS NULL THEN RAISE EXCEPTION 'not authenticated'; END IF;
  SELECT * INTO _pay FROM public.payments WHERE id=_payment_id FOR UPDATE;
  IF NOT FOUND THEN RAISE EXCEPTION 'payment not found'; END IF;
  IF _pay.status='approved' THEN PERFORM public.recalc_customer_balance(_pay.customer_id); RETURN; END IF;
  IF _pay.status<>'pending' THEN RAISE EXCEPTION 'only pending payments can be approved'; END IF;
  IF NOT (public.has_tenant_role(_pay.tenant_id,'manager') OR public.has_tenant_role(_pay.tenant_id,'admin')
      OR public.has_tenant_role(_pay.tenant_id,'accountant') OR public.has_tenant_role(_pay.tenant_id,'super_admin')) THEN
  ...
  FROM public.payments WHERE bill_id=_bill.id AND status='approved' AND id<>_pay.id;
  _new_paid := ROUND(_approved+_pay.amount,3);
  IF _new_paid > ROUND(COALESCE(_bill.total,0),3)+0.0005 THEN RAISE EXCEPTION 'approval would exceed bill total'; END IF;
  _new_status := CASE WHEN _new_paid >= ROUND(COALESCE(_bill.total,0),3)-0.0005 THEN 'paid'
                      WHEN _new_paid > 0 THEN 'partial' ELSE 'unpaid' END;
  UPDATE public.payments SET status='approved',approved_at=NOW(),approved_by=_uid,updated_at=NOW() WHERE id=_pay.id;
  INSERT INTO public.customer_ledger (
    tenant_id,customer_id,entry_type,reference_id,debit_amount,credit_amount,running_balance,description,posted_at,created_at
  ) VALUES (
  ...
  UPDATE public.water_bills SET paid_amount=_new_paid,status=_new_status,
    paid_at=CASE WHEN _new_status='paid' THEN COALESCE(paid_at,NOW()) ELSE paid_at END,updated_at=NOW()
  WHERE id=_bill.id;
...
BEGIN
  ...
  IF NOT (public.has_tenant_role(_pay.tenant_id,'manager') OR public.has_tenant_role(_pay.tenant_id,'admin')
      OR public.has_tenant_role(_pay.tenant_id,'accountant') OR public.has_tenant_role(_pay.tenant_id,'super_admin')) THEN RAISE EXCEPTION 'forbidden'; END IF;
  UPDATE public.payments SET status='rejected',rejected_at=NOW(),rejected_by=_uid,reject_reason=_reason,updated_at=NOW()
...
-- Every newly issued bill is posted once to the immutable customer ledger.
CREATE OR REPLACE FUNCTION public.tg_post_bill_to_ledger()
RETURNS TRIGGER LANGUAGE plpgsql SECURITY DEFINER
SET search_path = pg_catalog, public, pg_temp AS $$
BEGIN
  IF COALESCE(NEW.status,'unpaid') <> 'void' AND NEW.customer_id IS NOT NULL AND COALESCE(NEW.total,0)>0 THEN
    PERFORM public.acquire_customer_lock(NEW.tenant_id,NEW.customer_id);
    INSERT INTO public.customer_ledger (
      tenant_id,customer_id,entry_type,reference_id,debit_amount,credit_amount,running_balance,description,posted_at,created_at
    ) VALUES (
      NEW.tenant_id,NEW.customer_id,'bill',NEW.id,ROUND(NEW.total,3),0,0,
      FORMAT('إصدار فاتورة بمبلغ %s',ROUND(NEW.total,3)),COALESCE(NEW.issued_at,NEW.created_at,NOW()),NOW()
    ) ON CONFLICT (tenant_id,reference_id,entry_type) DO NOTHING;
...
CREATE TRIGGER trg_post_bill_to_ledger AFTER INSERT ON public.water_bills
FOR EACH ROW EXECUTE FUNCTION public.tg_post_bill_to_ledger();

-- Accountants/admins must be able to read the authoritative ledger for reconciliation.
DROP POLICY IF EXISTS tenant_select_customer_ledger ON public.customer_ledger;
CREATE POLICY tenant_select_customer_ledger ON public.customer_ledger FOR SELECT TO authenticated
USING (
  tenant_id=public.current_tenant_id() AND (
    public.has_tenant_role(tenant_id,'super_admin') OR public.has_tenant_role(tenant_id,'manager')
    OR public.has_tenant_role(tenant_id,'admin') OR public.has_tenant_role(tenant_id,'accountant')
    OR public.has_tenant_role(tenant_id,'collector') OR customer_id=auth.uid()
  )
);
